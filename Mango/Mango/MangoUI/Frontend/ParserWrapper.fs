...
let rec levenshtein_distance (a: string) (b: string) =
    if b.Length = 0 then
        ...
        1 + min 
            (min (levenshtein_distance a[1..] b) 
...
let parseString (source: string) =
    ...
    | SyntaxError boxed ->
        ...
        | Some (Parser.ID (name, (line, col))) ->
            ...
            Error $"Unknown identifier '{name}' at line {line}, col {col}, maybe you meant to write {most_likely_keyword}"
        | Some tok ->
            Error $"Unexpected token {tok}"
        | None ->
            Error "Syntax error at beginning of file"