...
let insertProperties element newProps =
    ...
    match element with
    | Button(_, pos) -> Button(Some newProps, pos)
    | TextBlock(_, pos) -> TextBlock(Some newProps, pos)
    | Border(_, element, pos) -> Border(Some newProps, element, pos)
    | Row(_, elements, pos) -> Row(Some newProps, elements, pos)
    | Column(_, elements, pos) -> Column(Some newProps, elements, pos)
    | _ -> element
