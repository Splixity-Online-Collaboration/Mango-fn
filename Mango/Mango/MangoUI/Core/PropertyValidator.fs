...
let validateProperties (element: AbSyn.UIElement) =
    ...
    | Some ps ->
        ...
        |> List.exists (fun p ->
            ...
            if Set.contains pk allowed then
                false
            else
...
let upsertProperty (newProp: AbSyn.Property) (props: AbSyn.Property list) =
    ...
    let withoutMatch, matchFound =
        ...
        |> List.fold
            (fun (acc, found) p ->
                if propertyKind p = kindToReplace then
                    (acc, true)
                else
                    (p :: acc, found))
            ([], false)
...
let createProp propertyKind (value: AbSyn.Value) = // using value instead of exp
    match propertyKind, value with
    | Hidden, AbSyn.Bool b -> AbSyn.Hidden (Some (b, (0,0)))
    | Label, AbSyn.String s -> AbSyn.Label (Some (s, (0,0)))
